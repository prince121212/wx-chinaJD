...
export async function GET(request: NextRequest) {
  try {
    ...
    const spuId = searchParams.get('id') || searchParams.get('spuId')
    
    if (!spuId) {
      return NextResponse.json(
        { success: false, msg: '缺少商品ID参数' },
        { status: 400 }
      )
    }

    console.log('获取产品详情 - 使用查询参数:', { spuId, userAgent })
    ...
    try {
      ...
      const formattedProduct = {
        ...
        spuStockQuantity: skus.reduce((total: number, sku: any) => total + (sku.stock || 0), 0),
        soldNum: 0,
        isPutOnSale: 1,
        categoryIds: product.categoryIds ? JSON.parse(product.categoryIds) : [],
